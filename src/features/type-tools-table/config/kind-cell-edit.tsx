...
export const KindToolsCell: FC<{ id: number }> = observer((props) => {
    ...
    return <>{meta === Meta.LOADING || meta === Meta.FETCHING ? <CircularProgress /> : elem?.name}</>
})

export const KindToolsEdit: FC<{ id?: null | number, row: MRT_Row<TypeTools> }> = observer((props) => {
    const { id, row } = props
    const { elem, meta } = useLocalObservable(() => new KindToolStore({ id }))
